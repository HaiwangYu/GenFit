...
G4eTrackRep::G4eTrackRep(int pdgCode, char propDir) :
...
double G4eTrackRep::extrapolateToPlane(StateOnPlane& state,
    const SharedPlanePtr& plane,
    bool stopAtBoundary,
    bool calcJacobianNoise) const {

  if (debugLvl_ > 0) {
    debugOut << "G4eTrackRep::extrapolateToPlane()\n";
  }
  ...
  double flightTime( 0. );
  double coveredDistance( Extrap(*(state.getPlane()), *plane, getCharge(state), getMass(state), isAtBoundary, state7, flightTime, fillG4eExtrapSteps, covPtr, false, stopAtBoundary) );
...
double G4eTrackRep::extrapolateToLine(StateOnPlane& state,
  ...
  if (debugLvl_ > 0) {
    debugOut << "G4eTrackRep::extrapolateToLine()\n";
  }
  ...
  while(true){
    if(++iterations == maxIt) {
      Exception exc("G4eTrackRep::extrapolateToLine ==> extrapolation to line failed, maximum number of iterations reached",__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
  ...
  if (fillG4eExtrapSteps) { // now do the full extrapolation with covariance matrix
  ...
  if (debugLvl_ > 0) {
    debugOut << "G4eTrackRep::extrapolateToLine(): Reached POCA after " << iterations+1 << " iterations. Distance: " << (poca_onwire-poca).Mag() << " cm. Angle deviation: " << dir.Angle((poca_onwire-poca))-TMath::PiOver2() << " rad \n";
  }
...
double G4eTrackRep::extrapToPoint(StateOnPlane& state,
  ...
  if (debugLvl_ > 0) {
    debugOut << "G4eTrackRep::extrapolateToPoint()\n";
  }
  ...
  if (G != NULL) {
    if(G->GetNrows() != 3) {
      Exception exc("G4eTrackRep::extrapolateToLine ==> G is not 3x3",__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
    dir = TMatrix(*G) * dir;
  ...
  while(true){
    if(++iterations == maxIt) {
      Exception exc("G4eTrackRep::extrapolateToPoint ==> extrapolation to point failed, maximum number of iterations reached",__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
  ...
  if (debugLvl_ > 0) {
    debugOut << "G4eTrackRep::extrapolateToPoint(): Reached POCA after " << iterations+1 << " iterations. Distance: " << (point-poca).Mag() << " cm. Angle deviation: " << dir.Angle((point-poca))-TMath::PiOver2() << " rad \n";
  }
...
double G4eTrackRep::extrapolateToCylinder(StateOnPlane& state,
    double radius,
    const TVector3& linePoint,
    const TVector3& lineDirection,
    bool stopAtBoundary,
  ...
  if (debugLvl_ > 0) {
    debugOut << "G4eTrackRep::extrapolateToCylinder()\n";
  }
  ...
  while(true){
    if(++iterations == maxIt) {
      Exception exc("G4eTrackRep::extrapolateToCylinder ==> maximum number of iterations reached",__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
    ...
    if(arg < 0) {
      Exception exc("G4eTrackRep::extrapolateToCylinder ==> cannot solve",__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
    ...
    if (debugLvl_ > 0) {
      debugOut << "G4eTrackRep::extrapolateToCylinder(); k = " << k << "\n";
    }
...
double G4eTrackRep::extrapolateToCone(StateOnPlane& state,
    double openingAngle,
  ...
  if (debugLvl_ > 0) {
    debugOut << "G4eTrackRep::extrapolateToCone()\n";
  }
  ...
  while(true){
    if(++iterations == maxIt) {
      Exception exc("G4eTrackRep::extrapolateToCone ==> maximum number of iterations reached",__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
    ...
    double arg = b*b - a*c;
    if(arg < -1e-9) {
      Exception exc("G4eTrackRep::extrapolateToCone ==> cannot solve",__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    } else if(arg < 0) {
      arg = 0;
    ...
    if (debugLvl_ > 0) {
      debugOut << "G4eTrackRep::extrapolateToCone(); k = " << k << "\n";
    }
...
double G4eTrackRep::extrapolateToSphere(StateOnPlane& state,
    double radius,
  ...
  if (debugLvl_ > 0) {
    debugOut << "G4eTrackRep::extrapolateToSphere()\n";
  }
  ...
  while(true){
    if(++iterations == maxIt) {
      Exception exc("G4eTrackRep::extrapolateToSphere ==> maximum number of iterations reached",__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
    ...
    if(arg < 0) {
      Exception exc("G4eTrackRep::extrapolateToSphere ==> cannot solve",__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
    double term = sqrt(arg);
    ...
    if (debugLvl_ > 0) {
      debugOut << "G4eTrackRep::extrapolateToSphere(); k = " << k << "\n";
    }
...
double G4eTrackRep::extrapolateBy(StateOnPlane& state,
    double step,
    bool stopAtBoundary,
    bool calcJacobianNoise) const {

  if (debugLvl_ > 0) {
    debugOut << "G4eTrackRep::extrapolateBy()\n";
  }
  ...
  while(true){
    if(++iterations == maxIt) {
      Exception exc("G4eTrackRep::extrapolateBy ==> maximum number of iterations reached",__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
    ...
    if (fabs(tracklength-step) < MINSTEP) {
      if (debugLvl_ > 0) {
        debugOut << "G4eTrackRep::extrapolateBy(): reached after " << iterations << " iterations. \n";
      }
      pos.SetXYZ(state7[0], state7[1], state7[2]);
      dir.SetXYZ(state7[3], state7[4], state7[5]);
      plane->setON(pos, dir);
...
TVector3 G4eTrackRep::getPos(const StateOnPlane& state) const {
  M1x7 state7;
  getState7(state, state7);
...
TVector3 G4eTrackRep::getMom(const StateOnPlane& state) const {
  M1x7 state7 = {{0, 0, 0, 0, 0, 0, 0}};
  getState7(state, state7);
...
void G4eTrackRep::getPosMom(const StateOnPlane& state, TVector3& pos, TVector3& mom) const {
  M1x7 state7 = {{0, 0, 0, 0, 0, 0, 0}};
  getState7(state, state7);
...
void G4eTrackRep::getPosMomCov(const MeasuredStateOnPlane& state, TVector3& pos, TVector3& mom, TMatrixDSym& cov) const {
...
TMatrixDSym G4eTrackRep::get6DCov(const MeasuredStateOnPlane& state) const {
  TMatrixDSym cov(6);
  transformPM6(state, *((M6x6*) cov.GetMatrixArray()));
...
double G4eTrackRep::getCharge(const StateOnPlane& state) const {

  if (dynamic_cast<const MeasurementOnPlane*>(&state) != NULL) {
    Exception exc("G4eTrackRep::getCharge - cannot get charge from MeasurementOnPlane",__LINE__,__FILE__);
    exc.setFatal();
    throw exc;
  }
...
double G4eTrackRep::getMomMag(const StateOnPlane& state) const {
  // p = q / qop
...
double G4eTrackRep::getMomVar(const MeasuredStateOnPlane& state) const {

  if (dynamic_cast<const MeasurementOnPlane*>(&state) != NULL) {
    Exception exc("G4eTrackRep::getMomVar - cannot get momVar from MeasurementOnPlane",__LINE__,__FILE__);
    exc.setFatal();
    throw exc;
  }
...
double G4eTrackRep::getSpu(const StateOnPlane& state) const {

  if (dynamic_cast<const MeasurementOnPlane*>(&state) != NULL) {
    Exception exc("G4eTrackRep::getSpu - cannot get spu from MeasurementOnPlane",__LINE__,__FILE__);
    exc.setFatal();
    throw exc;
  }
  ...
  if (auxInfo.GetNrows() == 2
      || auxInfo.GetNrows() == 1) // backwards compatibility with old G4eTrackRep
    return state.getAuxInfo()(0);
  else
    return 1.;
}

double G4eTrackRep::getTime(const StateOnPlane& state) const {
...
void G4eTrackRep::calcForwardJacobianAndNoise(const M1x7& startState7, const DetPlane& startPlane,
					     const M1x7& destState7, const DetPlane& destPlane) const {

  if (debugLvl_ > 0) {
    debugOut << "G4eTrackRep::calcForwardJacobianAndNoise " << std::endl;
  }

  if (G4eExtrapSteps_.size() == 0) {
    Exception exc("G4eTrackRep::calcForwardJacobianAndNoise ==> cache is empty. Extrapolation must run with a MeasuredStateOnPlane.",__LINE__,__FILE__);
    throw exc;
  }

  // The Jacobians returned from RKutta are transposed.
  TMatrixD jac(TMatrixD::kTransposed, TMatrixD(7, 7, G4eExtrapSteps_.back().jac7_.begin()));
  TMatrixDSym noise(7, G4eExtrapSteps_.back().noise7_.begin());
  for (int i = G4eExtrapSteps_.size() - 2; i >= 0; --i) {
    noise += TMatrixDSym(7, G4eExtrapSteps_[i].noise7_.begin()).Similarity(jac);
    jac *= TMatrixD(TMatrixD::kTransposed, TMatrixD(7, 7, G4eExtrapSteps_[i].jac7_.begin()));
  }
...
void G4eTrackRep::getForwardJacobianAndNoise(TMatrixD& jacobian, TMatrixDSym& noise, TVectorD& deltaState) const {
...
void G4eTrackRep::getBackwardJacobianAndNoise(TMatrixD& jacobian, TMatrixDSym& noise, TVectorD& deltaState) const {

  if (debugLvl_ > 0) {
    debugOut << "G4eTrackRep::getBackwardJacobianAndNoise " << std::endl;
  }

  if (G4eExtrapSteps_.size() == 0) {
    Exception exc("G4eTrackRep::getBackwardJacobianAndNoise ==> cache is empty. Extrapolation must run with a MeasuredStateOnPlane.",__LINE__,__FILE__);
    throw exc;
  }
...
std::vector<genfit::MatStep> G4eTrackRep::getSteps() const {

  // Todo: test

  if (G4eSteps_.size() == 0) {
    Exception exc("G4eTrackRep::getSteps ==> cache is empty.",__LINE__,__FILE__);
    throw exc;
  }

  std::vector<MatStep> retVal;
  retVal.reserve(G4eSteps_.size());

  for (unsigned int i = 0; i<G4eSteps_.size(); ++i) {
    retVal.push_back(G4eSteps_[i].matStep_);
  }
...
double G4eTrackRep::getRadiationLenght() const {

  // Todo: test

  if (G4eSteps_.size() == 0) {
    Exception exc("G4eTrackRep::getRadiationLenght ==> cache is empty.",__LINE__,__FILE__);
    throw exc;
  }
  ...
  for (unsigned int i = 0; i<G4eSteps_.size(); ++i) {
    radLen += G4eSteps_.at(i).matStep_.stepSize_ / G4eSteps_.at(i).matStep_.materialProperties_.getRadLen();
  }
...
void G4eTrackRep::setPosMom(StateOnPlane& state, const TVector3& pos, const TVector3& mom) const {

  if (state.getRep() != this){
    Exception exc("G4eTrackRep::setPosMom ==> state is defined wrt. another TrackRep",__LINE__,__FILE__);
    throw exc;
  }

  if (dynamic_cast<MeasurementOnPlane*>(&state) != NULL) {
    Exception exc("G4eTrackRep::setPosMom - cannot set pos/mom of a MeasurementOnPlane",__LINE__,__FILE__);
    exc.setFatal();
    throw exc;
  }

  if (mom.Mag2() == 0) {
    Exception exc("G4eTrackRep::setPosMom - momentum is 0",__LINE__,__FILE__);
    exc.setFatal();
    throw exc;
  }
...
void G4eTrackRep::setPosMom(StateOnPlane& state, const TVectorD& state6) const {
  if (state6.GetNrows()!=6){
    Exception exc("G4eTrackRep::setPosMom ==> state has to be 6d (x, y, z, px, py, pz)",__LINE__,__FILE__);
    throw exc;
  }
...
void G4eTrackRep::setPosMomErr(MeasuredStateOnPlane& state, const TVector3& pos, const TVector3& mom, const TVector3& posErr, const TVector3& momErr) const {
...
void G4eTrackRep::setPosMomCov(MeasuredStateOnPlane& state, const TVector3& pos, const TVector3& mom, const TMatrixDSym& cov6x6) const {

  if (cov6x6.GetNcols()!=6 || cov6x6.GetNrows()!=6){
    Exception exc("G4eTrackRep::setPosMomCov ==> cov has to be 6x6 (x, y, z, px, py, pz)",__LINE__,__FILE__);
    throw exc;
  }
...
void G4eTrackRep::setPosMomCov(MeasuredStateOnPlane& state, const TVectorD& state6, const TMatrixDSym& cov6x6) const {

  if (state6.GetNrows()!=6){
    Exception exc("G4eTrackRep::setPosMomCov ==> state has to be 6d (x, y, z, px, py, pz)",__LINE__,__FILE__);
    throw exc;
  }
...
void G4eTrackRep::setChargeSign(StateOnPlane& state, double charge) const {

  if (dynamic_cast<MeasurementOnPlane*>(&state) != NULL) {
    Exception exc("G4eTrackRep::setChargeSign - cannot set charge of a MeasurementOnPlane",__LINE__,__FILE__);
    exc.setFatal();
    throw exc;
  }
...
void G4eTrackRep::setSpu(StateOnPlane& state, double spu) const {
  state.getAuxInfo().ResizeTo(2);
  (state.getAuxInfo())(0) = spu;
}

void G4eTrackRep::setTime(StateOnPlane& state, double time) const {
  state.getAuxInfo().ResizeTo(2);
...
double G4eTrackRep::RKPropagate(M1x7& state7,
                        M7x7* jacobianT,
                        M1x3& SA,
                        double S,
                        bool varField,
  ...
  if (debugLvl_ > 0) {
    debugOut << "    G4eTrackRep::RKPropagate. Step = "<< S << "; quality EST = " << EST  << " \n";
  }
...
void G4eTrackRep::initArrays() const {
  ...
  G4eSteps_.reserve(100);
  G4eExtrapSteps_.reserve(100);
...
void G4eTrackRep::getState7(const StateOnPlane& state, M1x7& state7) const {

  if (dynamic_cast<const MeasurementOnPlane*>(&state) != NULL) {
    Exception exc("G4eTrackRep::getState7 - cannot get pos or mom from a MeasurementOnPlane",__LINE__,__FILE__);
    exc.setFatal();
    throw exc;
  }
...
void G4eTrackRep::getState5(StateOnPlane& state, const M1x7& state7) const {
...
void G4eTrackRep::transformPM7(const MeasuredStateOnPlane& state,
                              M7x7& out7x7) const {
...
void G4eTrackRep::calcJ_pM_5x7(M5x7& J_pM, const TVector3& U, const TVector3& V, const M1x3& pTilde, double spu) const {
  /*if (debugLvl_ > 1) {
    debugOut << "G4eTrackRep::calcJ_pM_5x7 \n";
    debugOut << "  U = "; U.Print();
    debugOut << "  V = "; V.Print();
...
void G4eTrackRep::transformPM6(const MeasuredStateOnPlane& state,
                              M6x6& out6x6) const {
...
void G4eTrackRep::transformM7P(const M7x7& in7x7,
                              const M1x7& state7,
...
void G4eTrackRep::calcJ_Mp_7x5(M7x5& J_Mp, const TVector3& U, const TVector3& V, const TVector3& W, const M1x3& A) const {

  /*if (debugLvl_ > 1) {
    debugOut << "G4eTrackRep::calcJ_Mp_7x5 \n";
    debugOut << "  U = "; U.Print();
    debugOut << "  V = "; V.Print();
    debugOut << "  W = "; W.Print();
...
void G4eTrackRep::transformM6P(const M6x6& in6x6,
                              const M1x7& state7,
...
//
bool G4eTrackRep::RKutta(const M1x4& SU,
  ...
  if(momentum < Pmin){
    ...
    sstream << "G4eTrackRep::RKutta ==> momentum too low: " << momentum*1000. << " MeV";
  ...
  while (fabs(S) >= MINSTEP || counter == 0) {
    ...
    if(++counter > maxNumIt){
      Exception exc("G4eTrackRep::RKutta ==> maximum number of iterations exceeded",__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
    ...
    if(Way > Wmax){
      std::ostringstream sstream;
      sstream << "G4eTrackRep::RKutta ==> Total extrapolation length is longer than length limit : " << Way << " cm !";
      Exception exc(sstream.str(),__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
    ...
    if (limits.getLowestLimit().first == stp_momLoss &&
      ...
      G4eSteps_.erase(G4eSteps_.end()-1);
      --G4eStepsFXStop_;
    ...
    if (fabs(deltaAngle) > AngleMax){
      std::ostringstream sstream;
      sstream << "G4eTrackRep::RKutta ==> Do not get to an active plane! Already extrapolated " << deltaAngle * 180 / TMath::Pi() << "°.";
      Exception exc(sstream.str(),__LINE__,__FILE__);
      exc.setFatal();
      throw exc;
    }
    ...
    if (counter > 3){
      if (S                            *G4eSteps_.at(counter-1).matStep_.stepSize_ < 0 &&
          G4eSteps_.at(counter-1).matStep_.stepSize_*G4eSteps_.at(counter-2).matStep_.stepSize_ < 0 &&
          G4eSteps_.at(counter-2).matStep_.stepSize_*G4eSteps_.at(counter-3).matStep_.stepSize_ < 0){
        Exception exc("G4eTrackRep::RKutta ==> Do not get closer to plane!",__LINE__,__FILE__);
        exc.setFatal();
        throw exc;
      }
    }
  ...
  if (limits.getLowestLimit().first == stp_plane) {
    ...
    if (jacobianT != NULL) {
      ...
      if (checkJacProj && G4eSteps_.size()>0){
        Exception exc("G4eTrackRep::Extrap ==> covariance is projected onto destination plane again",__LINE__,__FILE__);
        throw exc;
      }
...
double G4eTrackRep::estimateStep(const M1x7& state7,
                                const M1x4& SU,
                                const DetPlane& plane,
                                const double& charge,
                                double& relMomLoss,
...
TVector3 G4eTrackRep::pocaOnLine(const TVector3& linePoint, const TVector3& lineDirection, const TVector3& point) const {
...
double G4eTrackRep::Extrap(const DetPlane& startPlane,
...
void G4eTrackRep::checkCache(const StateOnPlane& state, const SharedPlanePtr* plane) const {

  if (state.getRep() != this){
    Exception exc("G4eTrackRep::checkCache ==> state is defined wrt. another TrackRep",__LINE__,__FILE__);
    exc.setFatal();
    throw exc;
  }

  if (dynamic_cast<const MeasurementOnPlane*>(&state) != NULL) {
    Exception exc("G4eTrackRep::checkCache - cannot extrapolate MeasurementOnPlane",__LINE__,__FILE__);
    exc.setFatal();
    throw exc;
  }
  ...
  if (plane &&
    ...
    for (unsigned int i=0; i<G4eSteps_.size(); ++i) {
      if (i == 0) {
        firstStep = G4eSteps_.at(0).matStep_.stepSize_;
        continue;
      }
      if (G4eSteps_.at(i).matStep_.stepSize_ * firstStep < 0) {
        if (G4eSteps_.at(i-1).matStep_.materialProperties_ == G4eSteps_.at(i).matStep_.materialProperties_) {
          G4eSteps_.at(i-1).matStep_.stepSize_ += G4eSteps_.at(i).matStep_.stepSize_;
        }
        G4eSteps_.erase(G4eSteps_.begin()+i, G4eSteps_.end());
      }
    }

    if (debugLvl_ > 0) {
        debugOut << "G4eTrackRep::checkCache: use cached material and step values.\n";
    }
  }
  else {

    if (debugLvl_ > 0) {
      debugOut << "G4eTrackRep::checkCache: can NOT use cached material and step values.\n";
...
double G4eTrackRep::momMag(const M1x7& state7) const {
  // FIXME given this interface this function cannot work for charge =/= +-1
  return fabs(1/state7[6]);
}


bool G4eTrackRep::isSameType(const AbsTrackRep* other) {
  if (dynamic_cast<const G4eTrackRep*>(other) == NULL)
    return false;
...
bool G4eTrackRep::isSame(const AbsTrackRep* other) {
...
void G4eTrackRep::Streamer(TBuffer &R__b)
{
   // Stream an object of class genfit::G4eTrackRep.

   //This works around a msvc bug and should be harmless on other platforms
   typedef ::genfit::G4eTrackRep thisClass;
   UInt_t R__s, R__c;
   if (R__b.IsReading()) {
