...
#ifndef genfit_G4eTrackRep_h
#define genfit_G4eTrackRep_h
 ...
 */
class G4eTrackRep : public AbsTrackRep {


 public:

  G4eTrackRep();
  G4eTrackRep(int pdgCode, char propDir = 0);

  virtual ~G4eTrackRep();

  virtual AbsTrackRep* clone() const {return new G4eTrackRep(*this);}
 ...
 private:
  ...
  mutable std::vector<G4eStep> G4eSteps_; //! RungeKutta steps made in the last extrapolation
  mutable int G4eStepsFXStart_; //!
  mutable int G4eStepsFXStop_; //!
  mutable std::vector<G4eExtrapStep> G4eExtrapSteps_; //! steps made in Extrap during last extrapolation

  mutable TMatrixD fJacobian_; //!
  mutable TMatrixDSym fNoise_; //!

  mutable bool useCache_; //! use cached G4eSteps_ for extrapolation
